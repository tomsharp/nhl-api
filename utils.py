...
def _last_game(team_id: int):
    r = requests.get(
        f"https://statsapi.web.nhl.com/api/v1/teams/{team_id}?expand=team.schedule.previous"
    )
    return r.json()
...
def _next_game(team_id: int, broadcasts: bool = True):
    request_str = (
        f"https://statsapi.web.nhl.com/api/v1/teams/{team_id}?expand=team.schedule.next"
    )
    if broadcasts == True:
        request_str = request_str + "&expand=schedule.broadcasts"
    r = requests.get(request_str)
...
def _upcoming_games(team_id: int, days_out: int = 1, broadcasts: bool = True):
    ...
    schedule = _get_schedule_raw(
        team_id, (date.today(), date.today() + timedelta(days=days_out)), broadcasts
    )
    return schedule
...
def next_game(team_id: int):
    ...
    broadcasts = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "broadcasts"
    ]
    home_team = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "teams"
    ]["home"]["team"]["name"]
    away_team = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "teams"
    ]["away"]["team"]["name"]
    home_record = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "teams"
    ]["home"]["leagueRecord"]
    away_record = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "teams"
    ]["away"]["leagueRecord"]

    # FIXME - grabbing for later
    radio_broadcasts = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][
        0
    ]["radioBroadcasts"]
    content = next_game["teams"][0]["nextGameSchedule"]["dates"][0]["games"][0][
        "content"
    ]
